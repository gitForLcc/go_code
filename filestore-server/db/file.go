...
func OnFileUploadFinished(filehash string, filename string, filesize int64, fileaddr string) bool {
	stmt, err := mydb.DBConn().Prepare(`insert ignore into tbl_file(file_sha1, file_name, file_size, file_addr, status)
	values(?, ?, ?, ?, 1)`)
	if err != nil {
	...
	if rf, err := ret.RowsAffected(); nil == err {
		if rf <= 0 {
			fmt.Printf("File with hash:%s hax been uploaded before", filehash)
		}
		return true
	}
	return false
